...
export function validateLeadSubmission(headers, formType, body = {}, options = {}) {
  ...
  if (isRateLimited(headers, formType) && !isLocal) {
  ...
  if (!hasTrustedFormOrigin(headers) && !isLocal) {
    return { ok: false, reason: 'origin' };
  }

  if (!Number.isFinite(submittedRenderedAt) && !isLocal) {
    return { ok: false, reason: 'timestamp' };
  }

  const expectedToken = signLeadForm(formType, submittedRenderedAt);
  if ((!submittedToken || submittedToken !== expectedToken) && !isLocal) {
    return { ok: false, reason: 'token' };
  }

  if (now - submittedRenderedAt < MIN_FORM_FILL_TIME_MS && !isLocal) {
    return { ok: false, reason: 'too_fast' };
  }

  if (now - submittedRenderedAt > LEAD_FORM_TTL_MS && !isLocal) {
